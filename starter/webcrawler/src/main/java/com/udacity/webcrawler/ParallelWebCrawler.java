...
final class ParallelWebCrawler implements WebCrawler {
  ...
  static final class CrawlTask extends RecursiveAction {
    ...
    protected void compute() {
      if (maxDepth == 0 || clock.instant().isAfter(deadline) || ignoredUrls.stream().anyMatch(pattern -> pattern.matcher(url).matches()) || !visitedUrls.add(url)) {
        return;
      }
      PageParser.Result result = parserFactory.get(url).parse();
      result.getWordCounts().forEach((key, value) -> counts.merge(key, value, Integer::sum));
